//
// Created by Gillett Hernandez on 4/25/18.
//
...
//    }
    for (int i = 0; i < 255; ++i) {
        palette[i] = make565(i/10,i/10,255-i);
    }
}
...
static void init_tables(void)
{
    init_palette();
    init_angles();
    topleft_x = -0.41666666f;
    topleft_y = -0.76666666f;
    window_width = 0.1666666666f;
    window_height = 0.166666666f;
    threshold = 1.0f;
}

static void render_frame( AndroidBitmapInfo*  info, void*  pixels, double  t )
{
    static float complex* field = NULL;
    if (field == NULL){
        field = (float complex*) malloc(info->height*info->width*sizeof(float complex));
    }
//    field = ;
    int yy;
...
//        Fixed      base = fixed_sin(yt1) + fixed_sin(yt2);
        ...
        for (xx = 0; xx < info->width; xx++) {
            float complex z = field[yy*info->width + xx];
            float r_x = topleft_x + window_width*xx/info->width;
            float r_y = topleft_y + window_height*yy/info->height;
            float complex new_z = z*z + (r_x + r_y *I);
            field[yy*info->width + xx] = new_z;
//            double value = (1-exp(-(pow(crealf(new_z),2) + pow(cimagf(new_z),2))));
            double value = (pow(crealf(new_z),2) + pow(cimagf(new_z),2));


            if (fabs(value) > threshold) {
                line[xx] = 0;
            } else {
                value = 255*(1-exp(-value));
                uint8_t idx = (uint8_t)(value);
                line[xx] = palette[idx]; //palette_from_fixed( / 4);
            }
        }
...
JNIEXPORT void JNICALL Java_com_nacly_fractal_FractalView_renderFractal(JNIEnv * env, jobject  obj, jobject bitmap,  jlong  time_ms, jint x, jint y, jint width, jint height)
{
    ...
    render_frame(&info, pixels, time_ms );
